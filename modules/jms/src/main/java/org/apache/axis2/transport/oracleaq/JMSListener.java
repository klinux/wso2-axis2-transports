...
public class JMSListener extends AbstractTransportListenerEx<JMSEndpoint> implements ManagementSupport,
     ...
     *
     * @param endpoint the oracleaq endpoint which is connected with the service
     */
    @Override
    protected void startEndpoint(JMSEndpoint endpoint) throws AxisFault {
        ...
        // First we will check whether oracleaq provider is started or not, as if not it will throw a continuous error log
        // If oracleaq provider not started we will wait for exponentially increasing time intervals, till the provider is started
        while (!connected) {
    ...
    private boolean checkJMSConnection(ServiceTaskManager stm) {
        ...
        try {
            //Closing the opened oracleaq connection
